...
CREATE OR REPLACE TABLE FUNCTION `autobet-470818.autobet.tf_sf_backtest_horse`(
  start_date DATE,
  end_date DATE,
  top_n INT64,
  cover_frac FLOAT64
...
WITH prods AS (
  ...
  strengths AS (
    ...
    FROM `autobet-470818.autobet.vw_superfecta_runner_strength_horse` s
  ...
  winners AS (
    SELECT
      r.event_id,
      MAX(IF(r.finish_pos=1, r.horse_id, NULL)) AS h1,
      MAX(IF(r.finish_pos=2, r.horse_id, NULL)) AS h2,
      MAX(IF(r.finish_pos=3, r.horse_id, NULL)) AS h3,
      MAX(IF(r.finish_pos=4, r.horse_id, NULL)) AS h4
    FROM `autobet-470818.autobet.hr_horse_runs` r
    WHERE r.finish_pos BETWEEN 1 AND 4
    GROUP BY r.event_id
    HAVING COUNTIF(r.finish_pos=1)=1 AND COUNTIF(r.finish_pos=2)=1
       AND COUNTIF(r.finish_pos=3)=1 AND COUNTIF(r.finish_pos=4)=1
  ),
  rr AS (
    SELECT

...
CREATE OR REPLACE TABLE FUNCTION `autobet-470818.autobet.tf_superfecta_coverage`(
  in_product_id STRING,
  top_n INT64
)
AS
WITH perms AS (
    SELECT * FROM `autobet-470818.autobet.tf_superfecta_perms`(in_product_id, top_n)
  ), ordered AS (
    SELECT

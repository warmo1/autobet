...
def calculate_pl_from_perms(
    *,
    ...
    bankroll: float,
    # Optional map: runner_id -> decimal odds (for crowd allocation)
    runner_odds: Optional[Dict[str, float]] = None,
    concentration: float,
    ...
    permutations. The `perms` items must contain a probability under key
    'probability' and a list of runner ids (strings or ints) under key 'ids'.

    Returns a structure matching calculate_pl_strategy()['pl_model'].
    """
    results: Dict[str, Any] = {"errors": [], "pl_model": None}
    if not perms:
        results["errors"].append("No permutations provided.")
        return results

    # Normalize and sort perms
    pl_permutations = []
    for it in perms:
        p = float(it.get("probability") or 0.0)
        ids = it.get("ids") or []
        # Construct a line display as space-joined ids
        line_str = " - ".join(str(x) for x in ids)
        # Build a runners_detail structure that carries id and odds (if provided)
        detail = [{"id": str(x), "odds": (runner_odds.get(str(x)) if runner_odds else None)} for x in ids]
        pl_permutations.append({"line": line_str, "runners_detail": detail, "probability": p})
    pl_permutations.sort(key=lambda x: x["probability"], reverse=True)

    ev_grid: List[Dict[str, Any]] = []
    ...
    # Crowd focus: higher mi means public over-weights poor lines; lower beta narrows crowd to favorites
    ...
    def _crowd_score(line):
        ...
        # Otherwise, use the line probability as a proxy.
        q = 1.0
        has_odds = False
    ...
    if target_profit > 0 and optimal_ev_scenario:
        eligible = [s for s in ev_grid if s['expected_profit'] >= target_profit]
        chosen = max(eligible, key=lambda x: x['expected_profit']) if eligible else optimal_ev_scenario
        hr = chosen['hit_rate']
        exp_ret = chosen['expected_profit'] + S
        fshare = (exp_ret / (hr * optimal_ev_scenario['net_pool_if_bet'])) if hr > 0 else 0.0
        base_scenario = {
            "lines_covered": chosen['lines_covered'], "hit_rate": hr, "expected_profit": chosen['expected_profit'],
            "expected_return": exp_ret, "f_share_used": fshare, "net_pool_if_bet": optimal_ev_scenario['net_pool_if_bet'],
            "total_stake": S,
        }

    # Final staking for display
    display_scenario = None
    staking_plan: List[Dict[str, Any]] = []
    if base_scenario:
        ...
        for i, line in enumerate(final_covered_lines):
            ...
            expected_profit += 0.0  # aggregated below after loop
            final_hit_rate += p_i
            fshare_weighted += p_i * f_i
...
def calculate_pl_strategy(
    *,
    ...
    base_scenario = optimal_ev_scenario
    if target_profit > 0 and optimal_ev_scenario:
        # Choose the scenario that maximizes EV while meeting the target,
        # not merely the first scenario that crosses the threshold.
        eligible = [s for s in ev_grid if s['expected_profit'] >= target_profit]
        chosen = max(eligible, key=lambda x: x['expected_profit']) if eligible else optimal_ev_scenario
        hr = chosen['hit_rate']
        exp_ret = chosen['expected_profit'] + S
